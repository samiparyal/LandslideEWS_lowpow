...
/* Sampling is paced by the sensor's DRDY pulse, so the ODR ladder IS the set of
   available cadences -- callers pick a rung, never a millisecond value.
   imu.c maps each rung to its ODR register value. Nothing here tracks elapsed
   time -- the detector reads the radio-timer clock directly. */
typedef enum {
