...
/* Sampling and sending are both virtual-timer driven, at exactly the
   POLL_INTERVAL_* / BROADCAST_* values in tilt_detector.h. The IMU DRDY
   interrupt path was removed: PB9 stays analog, nothing wakes on it. */
...
static void APP_BLE_TiltPollTimer_Callback(void *context)
{
    ...
    /* Re-armed by APP_BLE_TiltPoll_Task, which knows the resulting state.
       BISECT STEP 1: PRIO_0 to match the 130 uA build (was PRIO_1). */
...
void APP_BLE_Init(void)
{
  ...
  /* Virtual timers pace both sampling and sending. */
...
static void APP_BLE_TiltPoll_Task(void)
{
	...
	if (imu_poll() != 0)
	{
		/* Must re-arm here or sampling stops permanently. */
	...
	/* LED disabled entirely while chasing the current floor. To re-enable as a
	   "calibration in progress" indicator, restore this (LED is ACTIVE-LOW,
	   so RESET = lit, SET = dark):
	   HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin,
	                     (state == TILT_STATE_CALIBRATING) ? GPIO_PIN_RESET : GPIO_PIN_SET); */
	...
	else
	{
		...
		if (data_changed)
		{
			 uint8_t state_transitioned = (state != last_state) && (last_state != TILT_STATE_CALIBRATING);
	...
	/* Re-arm at exactly the configured POLL_INTERVAL_* for the new state. */
